...
Then read the row for `earnlog.xyz`. Or pull from
...
## 2026-05-25 — (one paragraph) Skip individual prompts with --<flag>; skip all with…
- **Status:** active
- **Hypothesis:** (one paragraph)
Skip individual prompts with --<flag>; skip all with --non-interactive.
  Paragraph prompts (1, 2, 4, 6, 9): finish with Enter twice or Ctrl-D.

Operator content for AI_AGENTS.md (press Enter to skip; section will render `(to be filled in)`)

  Summary — one paragraph: what this site is, what it does
  Hit Enter twice when done, or Ctrl-D. Enter twice immediately to skip. (Paste a multi-section response here to fill all prompts at once.)
